<?php
    ...
    class Lang
    {
        ...
        {
            global $info;
            $langs = list_langs('', !((bool) MODE));
            $this->log->write(2, 'info', 'Found ' . count($langs) . ' langs');
            $path = 'include/' . (MODE ? 'admin' : 'contents') . '/lang/';
            foreach ($langs as &$langtag)
            {
                if (file_exists($path . $langtag . '/' . $file . '.lang.php'))
                {
                    include($path . $langtag . '/' . $file . '.lang.php');
        ...
        private function getLangEntry(&$name)
        {
            ...
            elseif (isset($this->lang[CI_STD_LANG][$name]))
            {
                ...
                return $this->lang[CI_STD_LANG][$name];
            }
            else
            {
